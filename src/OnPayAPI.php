...
class OnPayAPI implements LoggerAwareInterface {
    ...
    /**
     * The OAuth state generated by the last authorize() call. Persist it
     * somewhere tied to the visitor's session and hand it back to
     * finishAuthorize(), or the callback cannot be told apart from one an
     * attacker made you follow.
     */
    public function getState(): ?string {
        return $this->oauth2Provider->getState();
    }

    /**
     * Exchanges an authorization code for an access token and stores it.
     *
     * Pass the state from the callback along with the one you stored before
     * redirecting, and they are compared before the code is spent. Omitting
     * them skips that check, which is only safe if you have already made the
     * comparison yourself.
     *
     * @throws TokenException on a state mismatch, or when the code is rejected
     * @throws ConnectionException
     */
    public function finishAuthorize(string $code, ?string $returnedState = null, ?string $expectedState = null): void {
        if (null !== $returnedState || null !== $expectedState) {
            if (null === $returnedState || null === $expectedState || !hash_equals($expectedState, $returnedState)) {
                throw new TokenException('OAuth state mismatch: this callback did not come from an authorization this client started.');
            }
        }
